...
static SAMPLE_OBJ_T t_g_ht               = {0};
static UINT16 ui2_g_sample_dbg_level = DBG_INIT_LEVEL_APP_SAMPLE;
...
static INT32 _sample_exit (
                    HANDLE_T                    h_app,
                    APP_EXIT_MODE_T             e_exit_mode
                    );
static INT32 _sample_process_msg (
                    HANDLE_T                    h_app,
                    UINT32                      ui4_type,
                    const VOID*                 pv_msg,
                    SIZE_T                      z_msg_len,
                ...
                )
{
    ...
    strncpy(pt_reg->s_name, SAMPLE_THREAD_NAME, APP_NAME_MAX_LEN);
    pt_reg->t_fct_tbl.pf_init                   = _sample_start;
    pt_reg->t_fct_tbl.pf_exit                   = _sample_exit;
    pt_reg->t_fct_tbl.pf_process_msg            = _sample_process_msg;
                    ...
                    )
{
    INT32 i4_ret;
    DBG_API_IN;
    memset(&t_g_ht, 0, sizeof(SAMPLE_OBJ_T));
    t_g_ht.h_app = h_app;
    ...
    ht_set_dbg_level(DBG_INIT_LEVEL_APP_SAMPLE);
#endif/* CLI_SUPPORT */
